...
void AudioThread::run()
{
    ...
    qint64 fake_pts = 0LL;
    sync_id = 0;
    int correctionCountDecreaseSpeed = 0;
    int correctionCountIncreaseSpeed = 0;
    while (!d.stop) {
        processNextTask();
        ...
        if (d.seek_requested) {
            ...
            qInfo("request seek audio thread");
        ...
        if (!pkt.isValid()) {
            ...
            //qInfo("eof pkt: %d valid: %d, aqueue size: %d, abuffer: %d %.3f %d, fake_duration: %lld", pkt.isEOF(), pkt.isValid(), d.packets.size(), d.packets.bufferValue(), d.packets.bufferMax(), d.packets.isFull(), fake_duration);
            ...
            if (pkt.isEOF()) {
                ...
                qInfo("audio thread gets an eof packet. pkt.pts: %.3f, d.render_pts0:%.3f", pkt.pts, d.render_pts0);
            }
            if (!pkt.isValid()) {
                if (pkt.pts >= 0) { // check seek first
                    qInfo("Invalid packet! flush audio codec context!!!!!!!! audio queue size=%d", d.packets.size());
                    QMutexLocker locker(&d.mutex);
                    Q_UNUSED(locker);
                    ...
                    qInfo("audio seek: %.3f, id: %d", d.render_pts0, sync_id);
                ...
                if (pkt.duration > 0) {
                    ...
                    //qInfo("get fake apkt: %.3f+%ul", pkt.pts, fake_duration);
                    continue;
                }
            }
            if (fake_duration > 0) {
                ...
                fake_duration -= ms;
                fake_pts += ms;
                //qInfo("fake_wait: %ul, fake_duration: %lld, delay: %.3f", ms, fake_duration, d.clock->delay());
        ...
        if (d.clock->initialValue() == 0) {
            qInfo("Update initial clock value from audio thread\n");
        ...
        if (is_external_clock && !pkt.isEOF()) {
            ...
            */
            double speedCorrectionFactor = 0.01;
            if((d.delay > 0 && d.delay < 0.025) && correctionCountIncreaseSpeed%25 == 0 || d.delay <= 0)
            {
                ao->setSpeed(qMin(1.0, ao->speed()+speedCorrectionFactor));
                correctionCountDecreaseSpeed = 0;
            }
            correctionCountIncreaseSpeed++;
            if (qAbs(d.delay) < 2.0) {
                ...
                if (d.delay > 0)
                {
                    //toFastInMS = d.delay*1000.0;
                    if(d.delay > 0.05 && correctionCountDecreaseSpeed%25 == 0)
                    {
                        ao->setSpeed(ao->speed()-speedCorrectionFactor);
                        correctionCountIncreaseSpeed = 0;
                    }
                    correctionCountDecreaseSpeed++;
                    //waitAndCheck(delayInMS, dts);
                    //msleep(delayInMS);
                }
                ...
                if (d.delay > 0) {
                    ...
                    qInfo("audio is too late compared with external clock. skip decoding. %.3f-%.3f=%.3f", dts, d.clock->value(), d.delay);
                    pkt = Packet(); //mark invalid to take next
                    continue;
                }
            }
        ...
        if (has_ao && dec->resampler()) {
            if (dec->resampler()->speed() != ao->speed()
                ...
                if (d.resample) {
                    qInfo() << "ao.format " << ao->audioFormat();
                    qInfo() << "swr.format " << dec->resampler()->outAudioFormat();
                    qInfo("decoder set speed: %.2f", ao->speed());
                    dec->resampler()->setInAudioFormat(ao->audioFormat());
            ...
            if (dec->resampler() && dec->resampler()->speed() != d.clock->speed()) {
                if (d.resample) {
                    qInfo("decoder set speed: %.2f", d.clock->speed());
        ...
        if (d.stop) {
            qInfo("audio thread stop before decode()");
            break;
        }
        //qInfo("apkt: %.3f, %lld %p", pkt.pts, pkt.asAVPacket()->pts, pkt.asAVPacket()->data);
        if (!dec->decode(pkt)) {
            qWarning("Decode audio failed. undecoded: %d", dec->undecodedSize());
            if (pkt.isEOF()) {
                qInfo("audio decode eof done");
                Q_EMIT eofDecoded();
                if (d.render_pts0 >= 0) {
                    qInfo("audio seek done at eof pts: %.3f. id: %d", pkt.pts, sync_id);
                    d.render_pts0 = -1;
        ...
        if (ao && !ao->isAvailable()) {
            AVCodecContext *avctx = (AVCodecContext*)d.dec->codecContext();
            AudioFormat af;
            ...
            if (af.isValid()) {
                ...
                ao->close();
                qInfo() << "AudioOutput format: " << ao->audioFormat() << "; requested: " << ao->requestedFormat();
                if (!ao->open()) {
                    qWarning("Audio: Failed to open ao\n");
                } else {
                    qInfo("Audio: Succesfully opened ao\n");
                }
                dec->resampler()->setOutAudioFormat(ao->audioFormat());
            }
        }
    ...
    d.packets.clear();
    qInfo("Audio thread stops running...");
}

void AudioThread::onNewFrameAvailable(Frame& baseFrame, AVPacket& pkt)
{
    DPTR_D(AudioThread);
    AudioFrame& frame = dynamic_cast<AudioFrame&>(baseFrame);
    AudioOutput *ao = 0;
    // first() is not null even if list empty
    if (!d.outputSet->outputs().isEmpty())
        ao = static_cast<AudioOutput*>(d.outputSet->outputs().first());
...
#if USE_AUDIO_FRAME
    if (!frame)
        return; //pkt data is updated after decode, no reset here
    ...
    if (d.render_pts0 >= 0.0) { // seeking
        ...
        if (frame.timestamp() < d.render_pts0) {
            qInfo("skip audio rendering: %f-%f", frame.timestamp(), d.render_pts0);
            return; //pkt data is updated after decode, no reset here
        }
        qInfo("audio seek finished @%.3f. id: %d", frame.timestamp(), sync_id);
        d.render_pts0 = -1.0;
        d.clock->syncEndOnce(sync_id);
    ...
    if (has_ao) {
        applyFilters(frame);
        //frame.setAudioResampler(d.dec->resampler()); //!!!
    ...
    //qInfo("frame samples: %d @%.3f+%lld", frame.samplesPerChannel()*frame.channelCount(), frame.timestamp(), frame.duration()/1000LL);
    while (decodedSize > 0) {
        if (d.stop) {
            qInfo("audio thread stop after decode()");
            break;
        }
