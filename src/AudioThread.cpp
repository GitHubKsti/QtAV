...
{
    //QtAV code with corrected timestamp handling
    //for KSTi custom but broken version see below at next #if block
#if 1
    DPTR_D(AudioThread);
        ...
        while (!d.stop) {
            ...
            if (d.clock->initialValue() == 0) {
                ...
                d.clock->setInitialValue(dts*1000*0.02);
            }
            ...
            if (skip_render) {
                d.clock->updateValue(pkt.pts*0.02*1000);
                /*
            ...
            if (d.render_pts0 >= 0.0) { // seeking
                d.clock->updateValue(frame.timestamp()*0.02*1000);
                ...
                if (has_ao && ao->isOpen()) {
                    ...
                    if (!is_external_clock && ao->timestamp() > 0) {//TODO: clear ao buffer
                       ...
                       // }
                        ...
                        d.clock->updateValue(ao->timestamp()*0.02*1000);
                    }
                } else {
...
#if 0 //KSTi custom stuff, is lip-sync but sounds like choppy mickey mouse!
    DPTR_D(AudioThread);
    //No decoder or output. No audio output is ok, just display picture
    if (!d.dec || !d.dec->isAvailable() || !d.outputSet)
        return;
