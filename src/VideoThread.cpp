...
void VideoThread::setEQ(int b, int c, int s)
{
    ...
    public:
        EQTask(VideoFrameConverter *c)
            ...
            , conv(c)
        {
            //qInfo("EQTask tid=%p", QThread::currentThread());
        }
        void run() {
...
void VideoThread::run()
{
    ...
    if (!d.dec || !d.dec->isAvailable() || !d.outputSet)
        return;
    resetState();
    AVDecoderFFmpeg* ffmpegDec = dynamic_cast<AVDecoderFFmpeg*>(d.dec);
    if (ffmpegDec)
    {
        ffmpegDec->setListener(this);
    }
    ...
    while (!d.stop) {
        ...
        if (d.seek_requested) {
            ...
            qInfo("request seek video thread");
            ...
            if (d.clock->syncId() > 0) {
                qInfo("video thread wait to sync end for sync id: %d", d.clock->syncId());
        ...
        if (pkt.isEOF()) {
            ...
            qInfo("video thread gets an eof packet.");
        } else {
            //qInfo() << pkt.position << " pts:" <<pkt.pts;
            //Compare to the clock
            if (!pkt.isValid()) {
                // may be we should check other information. invalid packet can come from
                wait_key_frame = true;
                qInfo("Invalid packet! flush video codec context!!!!!!!!!! video packet queue size: %d", d.packets.size());
                ...
                if (pkt.pts >= 0)
                    qInfo("video seek: %.3f, id: %d", d.render_pts0, sync_id);
        ...
        if (nb_no_pts > 5) {
            qInfo("the stream may have no pts. force fps to: %f/%f", d.force_fps < 0 ? -d.force_fps : 24, d.force_fps);
        ...
        const qreal dts = pkt.dts;//av_frame_get_best_effort_timestamp(pkt.asAVPacket()); //FIXME: pts and dts
        // TODO: delta ref time
        ...
        seeking = d.render_pts0 >= 0.0;
        if (seeking) {
            nb_dec_slow = 0;
            nb_dec_fast = 0;
        }
        //qInfo("nb_fast/slow: %d/%d. diff: %f, delay: %f, dts: %f, clock: %f", nb_dec_fast, nb_dec_slow, diff, d.delay, dts, clock()->value());
        if (d.delay < -0.5 && d.delay > diff) {
            if (!seeking) {
                // ensure video will not later than 2s
                if (diff < -2 || (nb_dec_slow > kNbSlowSkip && diff < -1.0 && !pkt.hasKeyFrame)) {
                    qInfo("video is too slow. skip decoding until next key frame.");
                    ...
                    nb_dec_slow++;
                    qInfo("frame slow count: %d. v-a: %.3f", nb_dec_slow, diff);
                }
            }
        } else {
            if (nb_dec_slow >= kNbSlowFrameDrop) {
                qInfo("decrease 1 slow frame: %d", nb_dec_slow);
        ...
        */
        ...
        if (qAbs(diff) < 0.5) {
            ...
            qInfo("delay %fs @%.3fs pts:%.3f", diff, d.clock->value(), pkt.pts);
            if (diff < 0) {
                ...
                qInfo("video too fast!!! sleep %.2f s, nb fast: %d, v_a: %.4f", s, nb_dec_fast, v_a);
        ...
        if (diff > 0.2 && diff < 1.0 && !seeking) {
            // can not change d.delay here! we need it to comapre to next loop
            qInfo("%f : DIFF > 0.2 AND < 1.0", diff);
            waitAndCheck(diff*1000UL, dts);
        }
        if (wait_key_frame) {
            if (!pkt.hasKeyFrame) {
                qInfo("waiting for key frame. queue size: %d. pkt.size: %d", d.packets.size(), pkt.data.size());
                pkt = Packet();
                v_a = 0;
                continue;
            }
        ...
        if (!seeking || pkt.pts - d.render_pts0 >= -0.05) { // MAYBE not seeking. We should not drop the frames near the seek target. FIXME: use packet pts distance instead of -0.05 (20fps)
            if (seeking)
                qInfo("seeking... pkt.pts - d.render_pts0: %.3f", pkt.pts - d.render_pts0);
            if (nb_dec_slow < kNbSlowFrameDrop) {
                if (dec_opt == &d.dec_opt_framedrop) {
                    qInfo("frame drop=>normal. nb_dec_slow: %d", nb_dec_slow);
                    dec_opt = &d.dec_opt_normal;
                }
            } else {
                if (dec_opt == &d.dec_opt_normal) {
                    qInfo("frame drop=>noref. nb_dec_slow: %d too slow", nb_dec_slow);
            ...
            if (seek_count > 0 && d.drop_frame_seek) {
                if (dec_opt == &d.dec_opt_normal) {
                    qInfo("seeking... pkt.pts - d.render_pts0: %.3f, frame drop=>noref. nb_dec_slow: %d", pkt.pts - d.render_pts0, nb_dec_slow);
                    dec_opt = &d.dec_opt_framedrop;
                }
            } else {
                seek_count = -1;
        ...
        if (dec != static_cast<VideoDecoder*>(d.dec)) {
            ...
            qInfo("decoder changed. decoding key frame");
        }
        if (dec_opt != dec_opt_old)
            dec->setOptions(*dec_opt);
        if (!dec->decode(pkt)) {
            d.pts_history.push_back(d.pts_history.back());
            qInfo("Decode video failed. undecoded: %d/%d", dec->undecodedSize(), pkt.data.size());
            if (pkt.isEOF()) {
                Q_EMIT eofDecoded();
                qInfo("video decode eof done. d.render_pts0: %.3f", d.render_pts0);
                if (d.render_pts0 >= 0) {
                    qInfo("video seek done at eof pts: %.3f. id: %d", d.pts_history.back(), sync_id);
                    d.render_pts0 = -1;
...
#endif
    d.packets.clear();
    qInfo("Video thread stops running...");
}

void VideoThread::onNewFrameAvailable(QtAV::Frame &origFrame, AVPacket &avPacket)
{
    DPTR_D(VideoThread);
    const char* pkt_data = NULL; // workaround for libav9 decode fail but error code >= 0
    AVCodecContext* codecContext = static_cast<AVCodecContext*>(d.dec->codecContext());
    if(!codecContext)
    {
        return;
    }
    const AVPacket* pAVPacket = &avPacket;
    Packet pkt = Packet::fromAVPacket(pAVPacket, av_q2d(codecContext->time_base));
    VideoFrame& frame = dynamic_cast<VideoFrame&>(origFrame);
    if (!frame.isValid()) {
        qInfo("invalid video frame from decoder. undecoded data size: %d", pkt.data.size());
    ...
    //qInfo("pts0: %f, pts: %f, clock: %d", d.render_pts0, pts, d.clock->clockType());
    if (d.render_pts0 >= 0.0) {
        ...
        d.render_pts0 = -1;
        //qInfo("video seek finished @%f. id: %d", pts, sync_id);
        d.clock->syncEndOnce(sync_id);
    ...
    //qInfo("force fps: %f dt: %d", d.force_fps, d.force_dt);
    if (d.force_dt > 0) {// && qFuzzyCompare(d.clock->speed(), 1.0)) {
        ...
        if (!seeking && display_wait > 0.0) {
            ...
            //qInfo("wait %f to display for pts %f-%f", display_wait, pts, clock()->value());
            if (display_wait < 1.0)
    ...
    if (!deliverVideoFrame(frame))
        return;
    //qInfo("clock.diff: %.3f", d.clock->diff());
    if (d.force_dt > 0)
